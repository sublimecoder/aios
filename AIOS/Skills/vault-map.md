...
2. Navigation half: scan the folder tree (+, AIOS, one dir per scope in layers.tsv, archive) → one line per top-level folder plus the fixed scope shape (me.md, notes/, content/, projects/, sources/), plus conventions (tags, wikilinks, privacy, publishing).
